...
class VacantView(ListAPIView):
    queryset = Listing.objects.order_by('-list_date').filter(vacant = False)
    permission_classes = (permissions.AllowAny, )
    serializer_class = ListingSerializer
    lookup_field = 'slug'
...
class SearchView(APIView):
    ...
    def post(self, request, format= None):
        ...
        for query in queryset:
            ...
            if count < int(has_photos):
                slug = query.slug
                queryset = queryset.exclude(slug__iexact=slug)

        vacant = data['vacant']
        queryset = queryset.filter(vacant__iexact=vacant)
